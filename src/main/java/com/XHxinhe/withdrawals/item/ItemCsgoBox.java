...
public class ItemCsgoBox extends Item {
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        tooltip.add(Text.translatable("tooltip.withdrawals.item.cs_box").formatted(Formatting.GRAY));
    ...
    public static Map<ItemStack, Integer> getItemGroup(ItemStack stack) {
        ...
        info.grade1.forEach(s -> itemsMap.put(ItemNbtUtil.fromNbtString(s), 1));
        info.grade2.forEach(s -> itemsMap.put(ItemNbtUtil.fromNbtString(s), 2));
        info.grade3.forEach(s -> itemsMap.put(ItemNbtUtil.fromNbtString(s), 3));
        info.grade4.forEach(s -> itemsMap.put(ItemNbtUtil.fromNbtString(s), 4));
        info.grade5.forEach(s -> itemsMap.put(ItemNbtUtil.fromNbtString(s), 5));
